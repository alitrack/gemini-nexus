...
- [ ] 1.3 Add `pendingMessages` Set to `PromptController` for tracking in-flight messages
- [ ] 1.4 Update message object structure to include optional `id` field (backward compatible)

## 2. Fix Message Index Calculation

- [ ] 2.1 Modify `handleStreamUpdate()` in `message_handler.js` to use pending message count for index calculation
- [ ] 2.2 Add duplicate index detection and logging in `handleStreamUpdate()`
- [ ] 2.3 Update streaming bubble creation to include message ID
- [ ] 2.4 Ensure pending messages are tracked when generation starts and removed when complete
...
- [ ] 4.1 Modify `history_manager.js` to include message IDs when saving
- [ ] 4.2 Update `appendAiMessage()` to accept and store message ID
...
- [ ] 5.1 Update `appendMessage()` in `message.js` to accept and store message ID on DOM element
- [ ] 5.2 Modify message update logic to use ID for finding correct message
- [ ] 5.3 Ensure `handleGeminiReply()` properly handles message IDs
- [ ] 5.4 Update regeneration flow to preserve and update message IDs correctly

## 6. Race Condition Detection and Logging

- [ ] 6.1 Add warning logs when duplicate indices are detected
- [ ] 6.2 Add warning logs when state divergence is detected
- [ ] 6.3 Log pending message queue state for debugging
...
- [ ] 8.3 Update BUGS-AND-IMPROVEMENTS.md to mark this issue as resolved
